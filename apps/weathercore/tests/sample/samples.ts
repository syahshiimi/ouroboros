import type {
  ...
  Temperature,
  UV,
} from "../../src/data-access/models/types";
...
export const sampleTemperatures: Temperature[] = sample.items
  .flatMap((item) =>
...
export const sampleStations: Stations[] = sample.metadata.stations.map(
  (station) => {
    return {
...
export const sampleHumidity: Humidity[] = humidity.items
  .flatMap((item) =>
    item.readings.map((reading) => ({
      station_id: reading.station_id,
      timestamp: item.timestamp,
...
export const sampleRainfallStations: Stations[] =
...
export const sampleRainfall: Rainfall[] = rainfall.items
  .flatMap((item) =>
    item.readings.map((reading) => ({
      station_id: reading.station_id,
      timestamp: item.timestamp,
